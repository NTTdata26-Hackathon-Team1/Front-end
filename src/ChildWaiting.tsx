    ...
    return (
        <div className="childwaiting-bg">
            ...
            style={{ 
                ...
                {"親がお題を入力中です".split("").map((char, i) => ( <span key={i} 
                    style={{ 
                        display: "inline-block", 
                        animation: `bounceChar 0.8s ${i * 0.08}s infinite`, }} > {char} </span> ))} <style> 
                {` @keyframes 
                    bounceChar { 0% { transform: translateY(0);} 30% { transform: translateY(-18px);} 60% { transform: translateY(0);} 100% { transform: translateY(0);} } `} 
                </style> 
            </div>
